...
export class SubagentOrchestrator {
  constructor(
    ...
    private compactThreshold: number = 0.9,
  ) {}
   ...
   * inline AgentLoop.
   *
   * `makeSink` is a FACTORY (not a sink) because creating the SSE writer binds the reply to
   * streaming (it commits the SSE headers on the first frame); the factory is only invoked AFTER
   * a successful mapping, so a failed mapping leaves the socket untouched for the inline fallback.
   */
  async start(args: {
    state: LoopStateData;
    sessionId: string;
  ...
  async runSubagentPhase(args: {
    ...
    /** ADR A-007 (passthrough-intacto): the client's request parameters, forwarded as-is. */
    passthrough?: Record<string, unknown>;
  }): Promise<{ content: string; reasoning: string; usage: TokenUsage | null; compactPending: boolean; tool_calls: ToolCall[]; finish_reason: string | null }> {
    ...
    if (state.compactPending) {
      ...
      return { content: COMPACT_PENDING_NOTICE, reasoning: '', usage: state.lastUsage, compactPending: true, tool_calls: [], finish_reason: null };
    }
    ...
    if (contextFull(result.usage, state.context_window_size, this.compactThreshold)) {
      ...
      return { content: COMPACT_PENDING_NOTICE, reasoning: result.reasoning ?? '', usage: result.usage ?? null, compactPending: true, tool_calls: [], finish_reason: null };
    }
    const pendingToolCalls = result.tool_calls ?? [];
    if (pendingToolCalls.length > 0) {
      // The subagent's model invoked the client's tools (the execute phase carries them, A-008):
      // relay the tool calls with finish_reason 'tool_calls' so the client EXECUTES them and
      // continues the SAME subagent session. The phase result stays deferred — the
      // continuation runs as a normal request and its last content wins
      // (recordSubagentResult). Storing the (empty) content now would hand the parent a
      // hollow result for a phase that is still running.
      args.sink?.emitToolCalls(pendingToolCalls);
      this.logger.info(
        `subagent phase: phase=${phase} agent_id=${agentId} ${Date.now() - t0}ms tool_calls=[${pendingToolCalls.map((t) => t.function.name).join(',')}], ` +
        `output_len=${(result.content ?? '').length} — phase result deferred until the subagent's tool results arrive`,
      );
      return { content: result.content ?? '', reasoning: result.reasoning ?? '', usage: result.usage ?? null, compactPending: false, tool_calls: pendingToolCalls, finish_reason: 'tool_calls' };
    }
    ...
    return { content: result.content ?? '', reasoning: result.reasoning ?? '', usage: result.usage ?? null, compactPending: false, tool_calls: [], finish_reason: null };
  }

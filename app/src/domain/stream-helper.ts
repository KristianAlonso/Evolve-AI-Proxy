...
export async function callWithStreaming(input: CallWithStreamingInput): Promise<CallWithStreamingResult> {
  ...
  if (typeof provider.completeStream === 'function') {
    ...
    // Derive the real finish reason from what was actually produced. Hardcoding 'stop' here
    // made the streaming passthrough relay `finish_reason: "stop"` even when the model had
    // requested native tool calls, so a delegating client (opencode subagent) treated the turn
    // as FINAL and never executed the relayed tool calls — the continuation died with an empty
    // phase result. OpenAI contract: a response carrying tool_calls finishes with 'tool_calls'.
    return {
      result: {
        ...
        refused: false,
        finish_reason: accToolCalls.length > 0 ? 'tool_calls' : 'stop',
        raw: {},
      },
      streamed: true,
    };

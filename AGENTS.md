...
### A-010: Identificadores de Caché OpenAI

`buildPassthrough(body, sessionId)` normaliza los identificadores de caché del cliente al campo estándar OpenAI **`prompt_cache_key`** (consume los alias `promptCacheKey`/`set_cache_key`; el resto de banderas de caché, p. ej. `prompt_cache_retention: "2h"`, viajan intactas vía passthrough). Sin valor del cliente se sintetiza `evolve_<x-session-id>`; sin sesión, no se inventa nada. Punto único de paso de todos los campos passthrough → llega a **todas** las llamadas upstream (parent, subagent, mapping, phase, passthrough). Verificado live: la captura upstream lleva `prompt_cache_key` en cada body.
- Detalle: [docs/adr/a-010-openai-cache-identifiers.md](./docs/adr/a-010-openai-cache-identifiers.md)

### A-011: Adopción Agnóstica del Resultado de Fase
...
- **Adopción agnóstica del resultado (A-011)**: antes del failover, `resume()` busca el resultado del spawn pendiente **en la conversación del padre** (sin depender de ningún plugin): (a) *strict* — un `tool` message cuyo `tool_call_id` es `spawn_<agent_id>` (el cliente ejecutó el spawn internamente, p. ej. un plugin con runner propio); (b) *loose* — primer `tool` message tras el último `assistant` con tool_calls (clientes que re-mintean ids). El contenido se adopta **tal cual** (si un plugin devolvió un error, ese error es el resultado — el proxy no lo clasifica, es transparente). Sin nada que adoptar, el failover de arriba se mantiene. Los prompts de spawn llevan línea `ACCEPTANCE: DONE when …` (gates de plugins tipo knowledge-gate). Verificado live con opencode + `opencode-swarm`: `planify → execute → evaluate → done`, `decision=complete`.
- **Endurecimiento**: si en un parent resume el resultado de la fase pendiente aún no llegó (el cliente bloqueó o no corrió el subagente, y no hay nada que adoptar), se **re-emite el mismo spawn** (mismo `agent_id`) sin avanzar rondas — un subagente bloqueado nunca gira el bucle con salidas vacías.
